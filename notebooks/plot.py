...
class PlotScores():
    def __init__(self, all_scores, opts):
        ...
        self.opts = opts
        self.mouse = widgets.Dropdown(options=['all', *opts['mice']], description='Mouse:')
    ...
    def plot(self, *args):
        ...
        if self.mouse.value == 'all':
            ...
            plt.errorbar(clusters_range, np.mean(scores, axis=0), yerr=np.std(scores, axis=0) / 
                        np.sqrt(len(self.opts['mice'])), label='all', color='tab:blue')
            [plt.plot(clusters_range, self.all_scores[mouse], alpha=0.25, label=mouse, color='tab:blue') for mouse in self.opts['mice']]
        else:
            plt.plot(clusters_range, self.all_scores[self.mouse.value], label=self.mouse.value)
